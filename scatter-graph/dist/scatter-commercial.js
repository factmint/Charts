...
define('config',['require'],function(Circle) {
	return {
		// Global settings
...
define('key',['snap', 'config', 'color-utils'],
...
define('scatter-key',['snap', 'config', 'key', 'color-utils'],
...
function(
	...
	return Snap.plugin(function(Snap, Element, Paper) {
		 ...
		 */
		Paper.prototype.scatterGraph = function(startX, startY, width, height, data, options) {
			...
			function showTooltip() {
				...
				var tooltipObject = new Tooltip(paper, colorGroupMap[this.data('metaInfo').group]);
					...
					[
						{
							...
							value: NumberUtils.renderValue(this.data('metaInfo').x)
						},
						{
							title: data.keys[2].title,
							value: NumberUtils.renderValue(this.data('metaInfo').y)
						}
					]
				);
            ...
            if (options.showTrendLines === "true") {
            	...
            	options.showTrendLines = false;
            	console.error("Show trend lines option is invalid, defaulting to false.");
            }

            var measureOneTitle = data.keys[1].title;
            var measureTwoTitle = data.keys[2].title;
            ...
            var yAxis = drawYAxis(data.yRange, options.xLabel);

            var yAxisOverflow = startX - yAxis.getBBox().x;
            yAxis.transform('t ' + yAxisOverflow + ' 0');

            var xAxis = drawXAxis(yAxis.startPoints[0].y, data.xRange, options.yLabel);
            var xAxisBBox = xAxis.getBBox();
...
function attributeToCamelCase(attribute) {
	...
	return function(table) {
		...
		paper.scatterGraph(canvasLeft, canvasTop, canvasWidth, canvasHeight, data, options);
...
require(['render'], function(render) {
    ...
    var tables = document.querySelectorAll('table.fm-scatter');
