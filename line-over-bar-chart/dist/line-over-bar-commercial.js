		...
		return Snap.plugin(function(Snap, Element, Paper) {
			...
			Paper.prototype.chart = function(startX, startY, width, height, data, options) {
				...
				function populateGraph() {
					...
					if (data.startPullout && data.endPullout) {
				...
				function drawKey(values) {
					if (width >= 250) {
						...
						var hasPullOut = data.startPullout && data.endPullout && options.pulloutTitle;
...
require(['render'], function(render) {
	...
	var tables = document.querySelectorAll('table.fm-line-over-bar');
