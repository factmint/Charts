...
define('data-processor',['config', 'number-utils', 'moment'],
	function(Config, NumberUtils, Moment) {
		...
		return {
			...
			//dateFormat, includeZero, prefix, suffix, abbreviateLabels) {
				...
				var VALUE_TIME_REGEX = /^\-?[0-9]+(:[0-5][0-9])(:[0-5][0-9])?(:[0-9]{3})?$/;
				...
				for (var rowIndex = 0; rowIndex < rows.length; rowIndex++) {
					...
					for (var columnIndex = 1; columnIndex < row.children.length; columnIndex++) {
						...
						if (column.match(VALUE_REGEX)) {
						...
						} else if (column.match(VALUE_TIME_REGEX)) {
							...
							if (components.length < 3) {
								components.push("00");
								data.formatDetail.seconds = false;
							}
							if (components.length < 4) {
								components.push("000");
								data.formatDetail.milliseconds = false;
							}
							
							var milliseconds = parseInt(components[3]);
							var seconds = parseInt(components[2]);
							var minutes = parseInt(components[1]);
							var hours = parseInt(components[0]);
							
							var yValue = milliseconds + (seconds * 1000) + (minutes * 60 * 1000) + (hours * 60 * 60 * 1000);

							ySeries.push({
								value: yValue,
								text: prefix + column + suffix,
								annotation: yAnnotation
							});
...
require(['render'], function(render) {
	...
	var tables = document.querySelectorAll('table.fm-line');
