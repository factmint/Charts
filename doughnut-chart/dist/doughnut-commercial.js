...
define('data-processor',['circle-utils'],
function(Circle) {
	return {
		...
		tableToJSON: function(container) {
			...
			for (var rowIndex = 0; rowIndex < rows.length; rowIndex++) {
				if (
					...
					if (rows[rowIndex].hasAttribute('data-fm-class-override')) {
						classOverride = rows[rowIndex].getAttribute('data-fm-class-override');
					}
					...
					if (classOverride) dataObject.classOverride = classOverride;
...
define('dashed-bracket',['snap', 'circle-utils'],
...
define('pie-segment',['snap', 'circle-utils'],
function(Snap,   Circle) {
	return Snap.plugin(function(Snap, Element, Paper) {
...
define('doughnut-segment',['snap', 'circle-utils'],
function(Snap,   Circle) {
	return Snap.plugin(function(Snap, Element, Paper) {
...
define('tooltip',['config', 'multitext'],
...
define('two-section-tooltip',['tooltip'],
function(Tooltip) {
...
define('central-data',['snap', 'config', 'number-utils'],
...
function(
	...
	return Snap.plugin(function(Snap, Element, Paper) {
		 ...
		 */
		Paper.prototype.doughnutChart = function(left, top, width, height, data, options) {
			...
			if (data[data.length - 1].hasOwnProperty('overflow')) {
				radius = (height - Config.TOP_PADDING_FOR_OVERFLOW - 2 * Config.TOP_PADDING_FOR_LABELS) / 2;
				yCenterOffset = Config.TOP_PADDING_FOR_OVERFLOW + Config.TOP_PADDING_FOR_LABELS
			} else {
				radius = (height - Config.TOP_PADDING_FOR_LABELS * 2) / 2;
				yCenterOffset = Config.TOP_PADDING_FOR_LABELS
			}

			if (2 * radius > (width - Config.SIDE_PADDING_FOR_LABELS * 2)) {
				radius = (width - Config.SIDE_PADDING_FOR_LABELS * 2) / 2;
			}

			var centerX = left + width / 2;
			var centerY = top + yCenterOffset + radius;
			...
			function drawSegmentDetailsInCenter() {
				...
				if (this.data('classOverride')) {
					centralData.addClass(this.data('classOverride'));
				}
			}
			...
			function activateSegment(segment) {
				...
				if (! segment.hasClass('fm-outer-segment')) {
					...
					if (segment.data('classOverride')) {
						overlaySegment.addClass(segment.data('classOverride'));
					}
				}
			}
			...
			function showTooltip() {
				if (paper.data('state') !== 2 || this.hasClass('fm-doughnut-segment')) {
					...
					var tooltipObject = new TwoSectionTooltip(paper, colorClass);
					...
					if (this.data('classOverride')) {
						tooltip.addClass(this.data('classOverride'));
					}
			...
			function drawOverflowSegments(segment, startAngle, totalAngle, overflowData) {
				...
				for (var dataIndex = 0; dataIndex < overflowData.length; dataIndex++) {
					...
					if (overflowData[dataIndex].hasOwnProperty('classOverride')) {
						doughnutSegment
							.addClass(overflowData[dataIndex].classOverride)
							.data('classOverride', overflowData[dataIndex].classOverride);
					}
			...
			for (var dataIndex = 0; dataIndex < data.length; dataIndex++) {
				...
				if (data[dataIndex].hasOwnProperty('classOverride')) {
					segment
						.addClass(data[dataIndex].classOverride)
						.data('classOverride', data[dataIndex].classOverride);
				}
...
function attributeToCamelCase(attribute) {
	...
	function drawPoweredByFactmint(previousSibling) {
		...
		previousSibling.parentElement.insertBefore(link, previousSibling.nextSibling);
	}
	...
	return function(table) {
		...
		paper.doughnutChart(canvasLeft, canvasTop, canvasWidth, canvasHeight, data, options);
		
		//COMMERCIALUSE drawPoweredByFactmint(svgNode);
	};
});
require.config({
	paths: {
...
require(['render'], function(render) {
	...
	if (! supportsSvg()) {
		...
		for (var tableIndex = 0; tableIndex < tables.length; tableIndex++) {
			tables[tableIndex].style.display = 'table';
		}
	} else {
