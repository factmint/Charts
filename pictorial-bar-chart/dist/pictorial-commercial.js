...
define('data-processor',['color-utils', 'number-utils', 'config'],
		...
		return {
			tableToJSON: function(container, options) {
				...
				for (var seriesIndex = 1; seriesIndex < series.length; seriesIndex++) {
					...
					var seriesColor = series[seriesIndex].hasAttribute('data-fm-glyph-class') ? series[seriesIndex].getAttribute('data-fm-glyph-class') : defaultColors[seriesIndex];
					data.series.push({
						name: seriesName,
						glyph: seriesGlyph,
						color: seriesColor
...
define('scale-utils',['number-utils'],
...
define('axis',['snap', 'tick-mark', 'number-utils'],
...
define('pictorial',[
	...
	function(
		...
		function showTooltip(paper, xPosition, yPosition, location, label, detail) {
			var tooltipObject = new Tooltip(paper, 'fm-datum-color-overflow');
		...
		return Snap.plugin(function(Snap, Element, Paper) {
			 ...
			 */
			Paper.prototype.pictorialBar = function(startX, startY, width, height, data, options) {
				...
				var gridXOrigin = (width + Config.LEFT_PADDING - Config.RIGHT_PADDING - (unitSquareLength * glyphsAcross)) / 2;
				var gridYOrigin = (height + Config.TOP_PADDING - Config.BOTTOM_PADDING - (unitSquareLength * data.groups.length)) / 2;
...
function attributeToCamelCase(attribute) {
	...
	return function(table) {
		...
		paper.pictorialBar(canvasLeft, canvasTop, canvasWidth, canvasHeight, data, options);
...
require.config({
	paths: {
		...
		"classList": "../../bower_components/classList.js/classList",
		...
		"pictorial-key": "../../bower_components/pictorial-key/dist/pictorial-key",
...
require(['render'], function(render) {
	...
	var tables = document.querySelectorAll('table.fm-pictorial-bar');
