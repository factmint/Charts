...
define('data-processor',[],function() {
	return {
...
define('bubble-key',['config', 'key', 'number-utils'],
...
define('multi-tier-tooltip',['tooltip', 'multitext'],
function(Tooltip) {
	...
	function MultiTierTooltip(paper, colorClass) {
...
define('bubble',[
	...
	function(
		...
		return Snap.plugin(function(Snap, Element, Paper) {
			 ...
			 */
			Paper.prototype.bubbleChart = function(startX, startY, width, height, dataSet, options) {
				var paper = this;
				// Keys
				...
				// Render the y axis
				yAxis = drawAxis(yRange, startX, startY + Config.TEXT_SIZE_SMALL, height, "vertical", options.yLabel);
				// Workout y overflow
				...
				// Render the x axis
				xAxis = drawAxis(xRange, startX + yOffsetX, yAxis.startPoints[0].y, width - yOffsetX - startX, "horizontal", options.xLabel);
				...
				function onBubbleMouseover(bubbleKey) {
					return function() {
						...
						if (bubbleKey.state.fixed.bubble !== this) {
							...
							var tooltipObject = new MultiLineTooltip(paper, this.data("colour"));

							tooltipObject.render(
								this.data("title"), [{
									title: areaKey,
									value: this.data("area value")
								}], [{
									title: xKey,
									value: this.data("x value")
								}, {
									title: yKey,
...
function attributeToCamelCase(attribute) {
	...
	return function(table) {
		...
		paper.bubbleChart(canvasLeft, canvasTop, canvasWidth, canvasHeight, data, options);
